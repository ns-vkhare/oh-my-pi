...
| `--agent <name>` | CLI flag | Appends the named task agent's prompt body (from `<cwd>/.omp/agents`, `~/.omp/agent/agents`, or the bundled set) as a block ahead of `--append-system-prompt`, and applies the agent's model, thinking level, tools, spawn policy, and autoloaded skills. Explicit `--model`/`--thinking`/`--tools` win. |
