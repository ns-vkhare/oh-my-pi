...
export function computeTwoColumnLayout(termWidth: number, leftMinContentWidth: number): BoxLayout {
	...
	// Floor at `leftMinContentWidth` so a pane whose fixed left content already
	// fits keeps its right column instead of collapsing one step too early.
	const desiredLeftCol = Math.max(
		Math.min(PREFERRED_LEFT_COL, Math.max(MIN_LEFT_COL, Math.floor(dualContentWidth * 0.35))),
		leftMinContentWidth,
	);
	const dualLeftCol =
