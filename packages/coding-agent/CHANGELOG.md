...
- Sessions dispatched fresh from `omp hub` now instruct the agent to create a new git worktree for the task by default (unless the prompt explicitly names a worktree/branch/tree), so parallel hub sessions don't collide in the same working tree. The worktree is created nested under the repo at `.worktrees/<slug>` (added to `.gitignore` so it stays untracked) rather than as a sibling of the repo. Resumed sessions are unaffected.
...
- Spawned subagent sessions now record both their parent's transcript path and their own registry id on their session header (`parentSession` — surfaced as `SessionInfo.parentSessionPath` — and `agentId`), so a subagent can identify its parent and be distinguished from a `/fork` child through the standard session API. This lets audit/telemetry consumers nest a subagent under the session that spawned it (rolling its turns into the parent's record) instead of treating it as an independent top-level session; top-level and forked sessions are unaffected.
- Added `omp --watch <sessionPath>`: a read-only spectator on a session another process owns, typically one the Slack bridge drives headlessly. It tails the transcript live without opening the session (no session manager, tools, or agent, so it never becomes a second writer), and proxies input to the owner over the bridge control socket — Enter steers the running turn, Ctrl+X interrupts it (leaving its subagents alone), and Ctrl+T takes the session over by parking the Slack task and handing the terminal to a real `omp --resume`. Esc quits with no side effects.
- Added `omp sessions [--json] [--dir <path>]`: list a project's sessions (title, age, status, path) as a plain table or JSON. The JSON mode is the machine seam the Slack bridge uses to browse the full session store from Slack (`sessions` / `resume <n>` in the bot DM).
- `omp hub` now badges sessions owned by the Slack bridge as `live · slack` (with `· working` while a turn is active), polling the bridge's control socket fail-soft alongside the disk scan — a bridge-owned session that fell out of the recent-sessions window is recovered so a live task can't disappear from the hub. Foregrounding such a row goes through the ownership handshake: the bridge parks its task (posting a handoff note to the Slack thread) before the terminal takes over.
- The Slack bridge's per-turn status message now shows what the agent is *thinking*, not just which tools it ran: each thinking block gets its own `💭` line in the turn timeline (rewritten in place as it streams, interleaved with the `⏵` tool labels), so a turn that reasons for a minute before touching a tool still shows movement. The excerpt is the newest complete reasoning-summary headline for models that emit them (gpt-5.x/codex), else the newest finished sentence, and nothing while a block is still a half-written stub. Slack traffic is unchanged — the same 2s throttle applies, and an unchanged timeline no longer re-sends the message at all.
- The Slack bridge now forwards a DM's attachments to the agent: uploaded files are downloaded with the bot token into `$TMPDIR/omp-slack-attachments/<ts>/` and their local paths appended to the prompt, so `run omp review this` with a PDF attached actually reads the PDF. Files hosted outside Slack (Drive, Box) and unfurled links are passed through as URLs, and a file that could not be fetched is named in the prompt with the reason rather than silently dropped. Requires the new `files:read` bot scope — re-paste `manifest.json` and reinstall the app if yours predates it.
- `omp --watch` can now watch the spectated session's subagents: `alt+a` opens a picker listing every task subagent and advisor transcript on disk (nested ones indented by depth), showing each one's live status and current task when the Slack bridge answers and its last-activity age otherwise, and Enter switches the live tail to that transcript while Esc returns to the main session. The header names the watched agent (`<task> ▸ <agent>`) and carries the subagent count, and Enter-to-steer, Ctrl+X, and Ctrl+T keep addressing the bridge-owned main session regardless of which transcript is on screen. The bridge control socket gained a `subagents` op to supply the per-subagent snapshots.
- omp now falls back to the plugins `pi` installed when it has none of its own by that name: each `~/.pi/agent/extensions/<name>` (package directory or bare `<name>.ts` module) and every entry of `~/.pi/agent/settings.json#packages` (a `git:` source resolves to the clone pi materialized at `~/.pi/agent/git/<host>/<owner>/<repo>`) is enumerated after both omp plugin roots, so its extensions, tools, hooks, commands, skills, and `.mcp.json` load through the normal surfaces. Strictly lowest precedence: an `omp plugin install`/`link` of the same package name always shadows the pi copy (nothing is loaded twice), and an explicit omp disable — `omp-plugins.lock.json` or a project's `plugin-overrides.json` — suppresses the fallback too, so removing an omp install can never silently re-enable a plugin through pi. Discovery is read-only and fail-soft: `omp plugin` commands never write into `~/.pi`, and an absent or unreadable pi tree contributes nothing.
- Added `omp --bare-system-prompt`: with `--system-prompt <file>`, send exactly that file as the system prompt — no `AGENTS.md` context files and no `PROJECT` footer (workstation block, cwd, workspace tree, standing directives). For workers whose prompt is the whole contract; the Slack bridge's routing classifier uses it to hand its local model a 3.3k-char prompt instead of a 58k-char one.
- The `bash` tool now exports the invoking session's identity to child processes — `PI_SESSION_FILE` (this session's transcript path) and `PI_SESSION_ID` — matching pi's behaviour, on the foreground, client-terminal, PTY, and background-job paths alike. Child tooling that starts its own agent sessions (e.g. `verify-review`, which spawns three `pi` judges) can read them to nest those sessions under the session that invoked it in cc-callbacks audits instead of recording them as unrelated top-level sessions. An `env` key the model supplies still wins over the injected value, and a session with no transcript on disk injects nothing.
- Added `omp --agent <name>`: run a top-level session as a discovered task agent (`<cwd>/.omp/agents`, `~/.omp/agent/agents`, or bundled). The agent's model, thinking level, tools, spawn policy, and autoloaded skills apply as they do for a task-tool spawn, and its prompt body is appended to the system prompt ahead of `--append-system-prompt`; explicit `--model`/`--thinking`/`--tools` flags still win. An unknown name lists the known agents and exits 2. Works in every mode, including `--mode rpc`. The Slack bridge's front-door router now uses it: a free-form DM is classified to an agent (`planner`, `scout`, `reviewer`, `librarian`, `implementer`, `sonic`, `orchestrate`) rather than a model role, and the agent file pins the model — `ORCHESTRATE_MODEL` and the router's `model` argument are gone.

### Changed

- Changed the `orchestrate` magic keyword to inject the resolved `orchestrate` agent definition (project `.omp/agents` > user `~/.omp/agent/agents` > bundled) instead of a separate hard-coded orchestration contract, so the agent file is the single source of truth for the orchestrator identity. omp now ships a bundled `orchestrate` agent (`spawns: "*"`, `@default` model, high thinking) as the default; the keyword adds no notice when the session already runs as that agent (`omp --agent orchestrate`).

### Fixed

- Fixed the Slack bridge answering a request in a brand-new thread instead of replying to the message that asked for it. A task's header, status, questions, and final answer now all land in a thread on the user's own message (or its existing root when the command was typed inside a thread), and a reply under a non-task bridge message — a `sessions` listing, a help message — is handled as a command instead of being silently dropped.
- Fixed the Slack bridge silently losing DMs sent while its Socket Mode connection was down or stale. Socket Mode has no backlog and Slack never retries, so a message typed during a restart — or into a socket a proxy had dropped without a FIN, which still reads as OPEN — was gone for good with no error anywhere. The bridge now pings every 30s so a dead peer surfaces as a close, and re-reads recent DM history on startup and every 2 minutes, running anything it never answered (bounded by `CATCHUP_WINDOW_MIN`, default 60, and a per-channel watermark in `state.json`, so a message is answered at most once). Finding a missed DM while the socket claims to be connected is treated as proof the socket is stale and forces a reconnect. Thread replies missed during an outage are still unrecoverable — Slack keeps them out of channel history.
- Fixed `omp hub` listing Slack-bridge sessions from *other* projects: the bridge reports every task it owns across every repo, and the hub merged them all into its rows, so a hub in one checkout showed tasks running in another. Bridge rows are now scoped to the hub's own session directory, exactly like the on-disk recent-sessions scan.
...
- Fixed a session dispatched fresh from `omp hub` aborting its own first turn ("Operation aborted") and dropping back to an idle row, forcing a manual retry. The in-session process tags its hub window with the session `.jsonl` path at startup, but that file is created lazily (not until the first assistant message), so the backend reaper — which kills any window whose tagged file `statSync`s to ENOENT as a "deleted session" — reaped the still-starting window mid-first-turn (SIGHUP → aborted turn) whenever a reap tick landed in the create gap. The reaper now only treats ENOENT as a deletion for windows whose file it has confirmed on disk at least once; a never-yet-seen file means the session is still creating it and the live window is left alone.
- Fixed the Slack bridge handshake attaching a terminal to a session the bridge might still own. A park request that timed out, closed early, or came back malformed was indistinguishable from "no bridge is running", so `omp --resume` (and the `--watch` take-over) treated an unresponsive daemon as a free session and pointed a second agent at the same transcript. Parking now reports *why* it failed: a refused connection still means the session is free, but an unanswered request fails closed with "Slack bridge is unresponsive" instead of guessing. The park deadline also now clears the daemon's own worst case (a subagent query plus its SIGTERM→SIGKILL grace), so a slow-but-healthy park is no longer read as a failure.
- Fixed a bare `omp` auto-resuming a session the Slack bridge was driving — that path skipped the ownership handshake entirely. It now runs the same check, and because a bare `omp` is not a request for one particular session, a busy or unverifiable candidate simply starts a new session instead of failing.
- Fixed pressing Esc during an `omp --watch` take-over silently stranding the session. Parking is not a query — the bridge stops its agent and posts a handoff note to the Slack thread before answering — so discarding a park that had already landed left a stopped session with no owner while the terminal reported "cancelled". A completed park is now always honoured; cancellation is only reported when nothing was parked.
- Fixed `omp hub` reading nothing back from tmux 3.6: the hub parses `tmux -F` output as tab-separated fields, but tmux replaces every byte outside printable ASCII — tab included — with `_`, so every row collapsed into a single field. Session windows, view sessions, and `omp hub list` all degraded silently (`hub list` showed mangled names with no project, session count, or activity). The format strings now use a printable delimiter.
